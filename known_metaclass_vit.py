...
Original file is located at
    https://colab.research.google.com/drive/1TCRXs3DC7cWCQIJvtqPlbMOXHJ5nH6Th
"""
...
def train_loop_C(epoch,model):
    ...
    for batch, (inputs, targets) in enumerate(train_dataloader):
        ...
        inputs = inputs.to(device)
        targets = targets.to(device)
        targetsC = targets.clone()
        for i in range(targets.size(0)):
          if targets[i] in [1, 9]:
            targetsC[i] = 0
          elif targets[i] in [0, 8]:
            targetsC[i] = 1
          elif targets[i] in [3, 5]:
            targetsC[i] = 2
          elif targets[i] in [2, 6]:
            targetsC[i] = 3
          elif targets[i] in [4, 7]:
            targetsC[i] = 4
